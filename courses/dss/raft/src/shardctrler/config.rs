...
use crate::shardctrler::errors::{Error, Result};
use crate::shardctrler::{client, server};
...
struct Servers {
    servers: Vec<Option<server::Node>>,
...
impl Config {
    ...
    pub fn make_client(&self, to: &[usize]) -> client::Clerk {
        ...
        for j in 0..self.n {
            ...
            ends.push(ShardCtrlerClient::new(cli));
    ...
    pub fn shutdown_server(&self, i: usize) {
        ...
        if let Some(kv) = servers.servers[i].take() {
            kv.kill();
        }
    }
    ...
    pub fn leader(&self) -> Result<usize> {
        let servers = self.servers.lock().unwrap();
        for (i, kv) in servers.servers.iter().enumerate() {
            if let Some(kv) = kv {
                if kv.is_leader() {
                    return Ok(i);
                }
